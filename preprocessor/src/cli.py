...
from preprocess import preprocess_file, preprocess_in_place
...
def main(argv: list[str] | None = None) -> int:
    ...
    # --in-place mode
    if args.in_place:
        changed = preprocess_in_place(str(in_path), indent=indent, check_only=args.check)
        if args.check:
            return 1 if changed else 0
        return 0

    # non in-place: output to file or stdout
    if args.output:
        out_path = Path(args.output)
        preprocess_file(str(in_path), str(out_path), indent=indent)
        return 0
